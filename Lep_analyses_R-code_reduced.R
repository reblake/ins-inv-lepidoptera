...
make_scatterplots <- function(df, title, xlab, xticklab){
                     ...
                     p <- ggplot(df, aes_string(x = df$area, y = df$value)) +
                          ...
                          theme_cowplot() + 
                          xlab(xlab) + # xlab(expression(paste("Area (km" ^ "2", ")"))) + 
                          ylab("No. Species") +
                          scale_x_log10() + 
                          scale_y_log10(breaks = c(1, 10, 100, 1000, 10000, 100000)) +
                          labs(title = title) 
  
                     if(xticklab == "labels"){ p1 <- p
                        }else{
                          if(xticklab == element_blank()){
                             p1 <- p + theme(axis.text.x = xticklab)
                             }
                          }
...
# This code prints errors, but the plots look correct, you it appears you can ignore the errors.
a1 <- make_scatterplots(alien1, "b)", NULL, element_blank()) 
a2 <- make_scatterplots(alien2, "d)", NULL, element_blank())
a3 <- make_scatterplots(alien3, "f)", expression(paste("Area (km" ^ "2", ")")), "labels")
n1 <- make_scatterplots(native1, "a)", NULL, element_blank())
n2 <- make_scatterplots(native2, "c)", NULL, element_blank())
n3 <- make_scatterplots(native3, "e)", expression(paste("Area (km" ^ "2", ")")), "labels")
